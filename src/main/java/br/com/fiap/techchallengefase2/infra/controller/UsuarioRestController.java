...
import br.com.fiap.techchallengefase2.infra.controller.model.request.UsuarioJson;
...
public class UsuarioRestController {
    ...
    public ResponseEntity<UsuarioOutputDTO> atualizar(
        ...
        var usuario = usuarioController.atualizarDadosParciaisUsuario(usuarioLogadoId, input);
        return ResponseEntity.ok(UsuarioOutputDTO.fromDomain(usuario));
    }
    ...
    public ResponseEntity<UsuarioOutputDTO> buscarPorId(
        ...
        var usuario = usuarioController.buscarUsuarioPorId(usuarioLogadoId, usuarioId);
        return ResponseEntity.ok(UsuarioOutputDTO.fromDomain(usuario));
    }

    @GetMapping
    public ResponseEntity<List<UsuarioOutputDTO>> buscarTodos(@RequestHeader("x-usuario-logado-id") Long usuarioLogadoId) {
        List<UsuarioOutputDTO> usuarios = usuarioController.buscarTodosUsuarios(usuarioLogadoId).stream()
